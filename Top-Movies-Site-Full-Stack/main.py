...
# Flask-Login -> framework care ajuta la login/sign up .
...
def add_movie():
    ...
    if form.validate_on_submit():
        ...
        response = requests.get(MOVIE_DB_SEARCH_URL,
                                params={"api_key": "c1da5421745044499c12a47565f7662f",
                                        "query": movie_title})
...
def find_movie():
    ...
    if movie_api_id:
        ...
        #Verifica daca filmul se afla deja in baza de date
...
def oscar_movies():
    ...
    for movie in oscar:
        ...
        response = requests.get(
            MOVIE_DB_SEARCH_URL,
            params={"api_key": "c1da5421745044499c12a47565f7662f", "query": movie}
        )
        data = response.json()["results"]
        if data:
            first_movie = data[0]
...
def top_movies_this_week():
    ...
    # In informatiile pe care le primesc despre filme am un camp popularity care arata
                        # cat de mare e interesul pentru film in momentul de fata.Mai jos il sorteaza dupa popularitate sa dea cele mai populare primele.
    sorted_movies = sorted(data, key=lambda x: x.get('popularity', 0), reverse=True)

    #Este o lista de vreo 30,40 de filme dar eu am pus sa imi dea doar primele 12 din ele ca sa nu fie lista prea lunga pe site.
